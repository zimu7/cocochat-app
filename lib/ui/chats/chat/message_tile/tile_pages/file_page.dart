...
import 'package:cocochat_app/ui/chats/chat/message_tile/tile_pages/pdf_page.dart';
import 'package:cocochat_app/ui/chats/chat/message_tile/tile_pages/video_page.dart';
...
class _FilePageState extends State<FilePage> {
                        ...
                        case FilePageStatus.openWithOtherApp:
                          return CupertinoButton.filled(
                              ...
                              child: Text(AppLocalizations.of(context)!
                                  .openWithOtherApps),
                              onPressed: () async {
                                SharePlus.instance.share(ShareParams(files: [XFile(_localFile!.path)]));
                              });
  ...
  void _shareFile(File file, String filename) async {
    ...
    SharePlus.instance.share(ShareParams(files: [XFile(tempFile.path)]));
  }
  ...
  Future<void> _download() async {
    ...
    try {
      final value = await widget.getFile(
        (p0, p1) {
      ...
      if (value != null) {
        ...
        if (!mounted) return;
        final title = AppLocalizations.of(context)!.filePageCantFindFile;
        final content = AppLocalizations.of(context)!.filePageCantFindFileContent;
        final okText = AppLocalizations.of(context)!.ok;
        showAppAlert(
            context: context,
            title: title,
            content: content,
      ...
      if (!mounted) return;
      final title = AppLocalizations.of(context)!.filePageDownloadFailed;
      final content = AppLocalizations.of(context)!.filePageDownloadFailedContent;
      final okText = AppLocalizations.of(context)!.ok;
      showAppAlert(
          context: context,
          title: title,
