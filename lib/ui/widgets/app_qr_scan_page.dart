  ...
  void _galleryQrScan() async {
    ...
    if (asset != null) {
      ...
      if (path != null) {
        ...
        final value = await cameraController.analyzeImage(path);
        if (value != null && value.barcodes.isNotEmpty) {
          App.logger.info("qr code found in image");
        } else {
          if (!mounted) return;
          final message = AppLocalizations.of(context)!.noQrCodeDetected;
